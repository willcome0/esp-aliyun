...
const iotx_linkkit_dev_meta_info_t subdevArr[EXAMPLE_SUBDEV_MAX_NUM] = {
    {
        ...
        "NCIRwiAYKQJoocZt",
        "slave1",
        "l5ktB4SHBiJ1sSfJ18ELJfCs6wiJwrsY"
     },
    {
        "a1i35Bp8m5w",
        "78WhxhY1Hhis5yqL",
...
int linkkit_main(void *paras)
{
    ...
    while (1)
    {
        ...
        /* Add subdev */
        user_example_ctx->auto_add_subdev = 1;
        ESP_LOGI("#my--", "允许添加：%d", user_example_ctx->permit_join);
            ...
            (user_example_ctx->auto_add_subdev == 1 || user_example_ctx->permit_join != 0))
        {
            ...
            {
                /* Add next subdev */
                if (example_add_subdev((iotx_linkkit_dev_meta_info_t *)&subdevArr[0]) == SUCCESS_RETURN)
                {
                    EXAMPLE_TRACE("设备 %s 添加成功", subdevArr[0].device_name);
                }
                else
                {
                    EXAMPLE_TRACE("设备 %s 添加失败", subdevArr[0].device_name);
                }
                user_example_ctx->subdev_index++;
...
// #define SLAVE_POST "/sys/" PRODUCT_KEY "/Slave1/thing/event/property/post"


// #define SLAVE_POST_REPLY "/sys/a1y0bg1jCBS/Slave1/thing/event/property/post_reply"
...
//             return -1;
//         }
//         ESP_LOGI("#MQTT--", "主题名：%s\n 载荷数据：%s\n 返回值：%d\r\n", TOPIC_ENENT_PROPERTY_POST, topic_msg.payload, rc);
...
//         if (cnt%2)
//         {
//             sprintf(msg_pub1, ALINK_BODY_FORMAT, "{\"LightStatus\":1}");
//         }
//         else
//         {
//             sprintf(msg_pub1, ALINK_BODY_FORMAT, "{\"LightStatus\":0}");
//         }
//         topic_msg.qos = IOTX_MQTT_QOS1;
//         topic_msg.retain = 0;
//         topic_msg.dup = 0;
//         topic_msg.payload = (void *)msg_pub1;
//         topic_msg.payload_len = strlen(msg_pub1);
//         rc = IOT_MQTT_Publish(g_p_mqtt_client, SLAVE_POST, &topic_msg);
//         if (rc < 0)
//         {
//             IOT_MQTT_Destroy(&g_p_mqtt_client);
//             ESP_LOGI("#从设备", "发送消息时发生错误");
//             return -1;
//         }
//         ESP_LOGI("#从设备", "主题名：%s\n 载荷数据：%s\n 返回值：%d", SLAVE_POST, topic_msg.payload, rc);
